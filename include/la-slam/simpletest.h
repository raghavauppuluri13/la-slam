...
#define MESSAGE_SPACE                                                          \
    10 * 1024 // default 10k of message space is reserved per test
#endif
...
#define STRING_EQ_PRINT_LENGTH                                                 \
    80 // max line length to show when comparing two strings
#endif
#if !defined(BASE_FIXTURE)
#define BASE_FIXTURE                                                           \
    TestFixture // use TestFixture as the test base class by default
#endif
#if !defined(ERROR_ACTION)
#define ERROR_ACTION // Defined any code to run on error. You can use this to
                     // debug break or do anything really
#endif
...
// TODO: Use a global template function for conversion so users can override
// this
//---------------------------------------------------------------------------------
struct TempString {
...
inline TempString TypeToString(unsigned char value) {
    return TypeToString((unsigned int)value);
}
inline TempString TypeToString(short value) { return TypeToString((int)value); }
inline TempString TypeToString(unsigned short value) {
    return TypeToString((unsigned int)value);
}
inline TempString TypeToString(char *value) {
    return TypeToString((char const *)value);
}
inline TempString TypeToString(void *value) {
    return TypeToString((void const *)value);
}

// if nothing specified then print some memory
template <typename T> TempString TypeToString(T const &) {
    return TempString();
}

template <typename T> TempString TypeToString(T const *pointer) {
    return pointer == nullptr
               ? TypeToString((void const *)pointer)
               : TypeToString((void const *)pointer, *TypeToString(*pointer));
}

template <typename T> TempString TypeToString(T *pointer) {
    return TypeToString((T const *)pointer);
}

inline TempString TypeToStringFallback(TempString string,
                                       char const *fallback) {
    return (*string)[0] ? string : TempString(fallback);
}
...
class TestFixture {
  public:
    ...
    bool TestStrings(char const *left, char const *right, char const *prefix,
    ...
    TestError const *GetFirstError() const {
        return (TestError *)myMessageSpace;
    }
    ...
    void SetPrintMethod(PrintMethod aPrintMethod) {
        myPrintMethod = aPrintMethod;
    }
    ...
    static bool ExecuteAllTests(char const *groupFilter = nullptr,
                                char const *nameFilter = nullptr,
                                OutputMode output = Normal);
    static bool ExecuteAllTests(OutputMode output) {
        return ExecuteAllTests(nullptr, nullptr, output);
    }

    static bool ExecuteTestGroup(char const *groupFilter,
                                 OutputMode output = Normal) {
        return ExecuteAllTests(groupFilter, nullptr, output);
    }
...
#define DEFINE_TEST_GF(name, group, fixture)                                   \
    DEFINE_TEST_FULL(name, group, fixture)

//---------------------------------------------------------------------------------
// Utils
//---------------------------------------------------------------------------------
template <typename T> T TestDifference(T const &a, T const &b) {
    return a > b ? a - b : b - a;
}
...
//---------------------------------------------------------------------------------
#define TEST_TYPE_TO_STRING(var, arg)                                          \
    *TypeToStringFallback(TypeToString(var), STR(arg))
#define TEST_ERROR_PREFIX_                                                     \
    __FILE__ "(" STR(__LINE__) "): Condition [%s] Failed. "
#define TEST_ERROR_(message, ...)                                              \
    do {                                                                       \
        TestFixture *__fx = TestFixture::GetCurrentTest();                     \
        __fx->AddError();                                                      \
        __fx->LogMessage(TEST_ERROR_PREFIX_ message, ##__VA_ARGS__);           \
        ERROR_ACTION;                                                          \
    } while (0)
#define TEST_BEGIN_(a)                                                         \
    do {                                                                       \
    auto const &test_value_ = a
#define TEST_CHECK_(cond, condtext, message, ...)                              \
    do {                                                                       \
        TestFixture::GetCurrentTest()->AddTest();                              \
        if (!(cond))                                                           \
            TEST_ERROR_(message, condtext, ##__VA_ARGS__);                     \
    } while (0)
#define TEST_END_                                                              \
    }                                                                          \
    while (0)
...
#define TEST_OPERATOR(a, b, op1, op2)                                          \
    TEST_BEGIN_(a);                                                            \
    TEST_CHECK_((test_value_)op1(b), STR(a) " " STR(op1) " " STR(b),           \
                "'%s' " STR(op2) " '%s'", TEST_TYPE_TO_STRING(test_value_, a), \
