...
TempString TypeToString(int value) {
    ...
    if (TestFixture::GetCurrentTest()->GetPrintMethod() ==
        TestFixture::PrintHexadecimal)
...
TestFixture::TestFixture()
    : myNextTest(nullptr), myNextError(nullptr), myNumTestsChecked(0),
      myNumErrors(0), myPrintMethod(PrintDefault) {
...
//---------------------------------------------------------------------------------
static void locCopyStringWithElipse(char dest[STRING_EQ_PRINT_LENGTH],
                                    char const *string, size_t offset = 0) {
    ...
    for (; i < STRING_EQ_PRINT_LENGTH - 1 && start[i]; ++i) {
        if (i < 3 && start > string)
            ...
            dest[i] = '\\'; // simply replace this with '\', we're just aiming
                            // for a general idea not an exact representation
        else
            dest[i] = start[i];
    }
...
//---------------------------------------------------------------------------------
// Instead of just check for error and printing the string, try go be smart
// about how the information is written out:
// ... quick brown fox jumps over ...
//                      ^
// ... quick brown fox jamps over ...
//---------------------------------------------------------------------------------
bool TestFixture::TestStrings(char const *left, char const *right,
                              char const *prefix, char const *condition) {
    AddTest();
    if (left == right) {
        return true;
    }
...
//---------------------------------------------------------------------------------
// Write error into current error object and advance pointer if there's still
// enough space
//---------------------------------------------------------------------------------
void TestFixture::LogMessage(char const *string, ...) {
    uintptr_t spaceLeft =
        (myMessageSpace + MESSAGE_SPACE) - (char *)myNextError;
    ...
    // if there isn't a reasonable amount of space left then just advance to end
    // and stop printing errors
    if (printedChars < (int)(spaceLeft - sizeof(TestError) - 64)) {
        uintptr_t nextOffset = (uintptr_t(myNextError->message) + printedChars +
                                sizeof(TestError) * 2 - 1);
...
static bool locExecuteTest(TestFixture *test, TestFixture::OutputMode output) {
    ...
    if (output == TestFixture::Verbose) {
        TestFixture::Printf("Running [%s/%s]", test->TestGroup(),
                            test->TestName());
        start = clock();
    }
    ...
    if (output != TestFixture::Silent) {
        ...
        for (TestError const *err = test->GetFirstError(),
                             *e = test->GetLastError();
             err != e; err = err->next) {
...
bool TestFixture::ExecuteAllTests(char const *groupFilter,
                                  char const *nameFilter, OutputMode output) {
    if (output != Silent) {
    ...
    for (auto i = TestFixture::GetFirstTest(); i; i = i->GetNextTest()) {
        bool matchGroup =
            groupFilter == nullptr || strcmp(groupFilter, i->TestGroup()) == 0;
        bool matchName =
            nameFilter == nullptr || strcmp(nameFilter, i->TestName()) == 0;
    ...
    if (output != Silent) {
        if (count == 0)
            ...
            Printf("%d Tests finished, %d of %d assertions failed. Some tests "
